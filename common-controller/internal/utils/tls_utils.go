...
func GetTrustedCertPool(truststoreLocation string) *x509.CertPool {
	onceTrustedCertsRead.Do(func() {
		...
		error := filepath.Walk(truststoreLocation, func(path string, info os.FileInfo, err error) error {
			if err != nil {
		...
		if error != nil {
			logger.LoggerTLSUtils.Warn("Error walking the path %s", truststoreLocation, error)
		}
	})
	return caCertPool
}
...
func GetKeyLocations() (string, string, string) {
	...
	publicKeyLocation := conf.CommonController.Keystore.CertPath
	privateKeyLocation := conf.CommonController.Keystore.KeyPath
	truststoreLocation := conf.CommonController.Truststore.Location
