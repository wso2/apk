...
func (cb *Callbacks) OnStreamRequest(id int64, request *discovery.DiscoveryRequest) error {
	...
	if request.ErrorDetail != nil {
		logger.LoggerEnforcerXdsCallbacks.ErrorC(logging.PrintError(logging.Error1400, logging.CRITICAL, "Stream request for type %s on stream id: %d Error: %s", request.GetTypeUrl(), id, request.ErrorDetail.Message))
	}
