...
/**
 * Util class for import functionalities
 */
public class ImportUtils {
    ...
    public static APIKey getAPIKeyFromApplicationKeyDTO(ApplicationKeyDTO applicationKeyDto) {
        ...
        String urlDecodeBase64ConsumerSecret = Arrays.toString(
                Base64.decodeBase64(applicationKeyDto.getConsumerSecret()));
        apiKey.setType(String.valueOf(applicationKeyDto.getKeyType()));
        apiKey.setConsumerKey(applicationKeyDto.getConsumerKey());
        apiKey.setConsumerSecret(urlDecodeBase64ConsumerSecret);
        ...
        if (apiKey.getGrantTypes() != null && (apiKey.getGrantTypes()
                .contains(GRANT_TYPE_IMPLICIT) || apiKey.getGrantTypes().contains(GRANT_TYPE_CODE))) {
        ...
        for (ExportedSubscribedAPI subscribedAPI : subscribedAPIs) {
            ...
            if (!StringUtils.isEmpty(tenantDomain)) {
                ...
                if (StringUtils.isNotEmpty(uuidFromIdentifier)) {
                    ...
                    if (isTierAvailable(targetTier,
                            apiTypeWrapper) && apiTypeWrapper.getStatus() != null && APIConstants.PUBLISHED.equals(
                            apiTypeWrapper.getStatus())) {
                        ...
                        // or when update skip subscriptions that already exists
                        if (update == null || !update || !apiConsumer.isSubscribedToApp(subscribedAPI.getApiId(),
                                userId, application.getId())) {
                            apiConsumer.addSubscription(apiTypeWrapper, userId, application);
                        }
                    } else {
                        log.error(
                                "Failed to import Subscription as API/API Product " + apiIdentifier.getName() + "-"
                                        + apiIdentifier.getVersion() + " as one or more tiers may " + "be unavailable "
                                        + "or the API/API Product may not have been published ");
                        skippedAPIList.add(subscribedAPI.getApiId());
                    }
                } else {
                    log.error(
                            "Failed to import Subscription as API " + apiIdentifier.getName() + "-"
    ...
    private static boolean isTierAvailable(String targetTierName, ApiTypeWrapper apiTypeWrapper) {
        ...
        if (api != null && !apiTypeWrapper.isAPIProduct()) {
            log.error("Tier:" + targetTierName + " is not available for API " + api.getId()
                    .getApiName() + "-" + api.getId().getVersion());
        } else if (apiProduct != null) {
            log.error("Tier:" + targetTierName + " is not available for API Product " + apiProduct.getId()
    ...
    public static void addApplicationKey(String username, Application application, ApplicationKeyDTO applicationKeyDTO,
        ...
        if (!update) {
            ...
            apiConsumer.updateAuthClient(username, application, applicationKeyDTO.getKeyType().toString(),
                    applicationKeyDTO.getCallbackUrl(), null, null, null,
                    application.getGroupId(), jsonParams, applicationKeyDTO.getKeyManager());
        }
    }
}
