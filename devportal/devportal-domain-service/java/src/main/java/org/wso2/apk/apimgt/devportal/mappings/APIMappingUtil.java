...
package org.wso2.apk.apimgt.devportal.mappings;
...
import org.wso2.apk.apimgt.devportal.dto.APIInfoAdditionalPropertiesInnerDTO;
import org.wso2.apk.apimgt.devportal.dto.APIInfoDTO;
import org.wso2.apk.apimgt.devportal.dto.APIListDTO;
import org.wso2.apk.apimgt.devportal.dto.APITiersInnerMonetizationAttributesDTO;
import org.wso2.apk.apimgt.devportal.dto.APIMonetizationInfoDTO;
import org.wso2.apk.apimgt.devportal.dto.APIOperationsDTO;
import org.wso2.apk.apimgt.devportal.dto.APITiersInnerDTO;
import org.wso2.apk.apimgt.devportal.dto.APIEndpointURLsInnerURLsDTO;
import org.wso2.apk.apimgt.devportal.dto.AdvertiseInfoDTO;
import org.wso2.apk.apimgt.devportal.dto.PaginationDTO;
import org.wso2.apk.apimgt.devportal.dto.RatingListDTO;
import org.wso2.apk.apimgt.devportal.dto.ScopeInfoDTO;
import org.wso2.apk.apimgt.devportal.dto.APIDTO;
import org.wso2.apk.apimgt.devportal.dto.APIBusinessInformationDTO;
import org.wso2.apk.apimgt.devportal.dto.RatingDTO;
...
public class APIMappingUtil {
    ...
    public static APIDTO fromAPItoDTO(API model, String organization) throws APIManagementException {
        ...
        List<APITiersInnerDTO> tiersToReturn = new ArrayList<>();
//        int tenantId = 0;
...
//        }
        ...
        for (Tier currentTier : apiTiers) {
            if (!deniedTiers.contains(currentTier.getName())) {
                APITiersInnerDTO APITiersInnerDTO = new APITiersInnerDTO();
                APITiersInnerDTO.setTierName(currentTier.getName());
                APITiersInnerDTO.setTierPlan(currentTier.getTierPlan());
                ...
                if (APIConstants.COMMERCIAL_TIER_PLAN.equalsIgnoreCase(currentTier.getTierPlan())) {
                    APITiersInnerMonetizationAttributesDTO monetizationAttributesDTO = new APITiersInnerMonetizationAttributesDTO();
                    if (!currentTier.getMonetizationAttributes().isEmpty()) {
                    ...
                    APITiersInnerDTO.setMonetizationAttributes(monetizationAttributesDTO);
                }
                tiersToReturn.add(APITiersInnerDTO);
            }
        }
        ...
        if (model.getAdditionalProperties() != null) {
            ...
            List<APIInfoAdditionalPropertiesInnerDTO> additionalPropertiesList = new ArrayList<>();
            for (Object propertyKey : additionalProperties.keySet()) {
                APIInfoAdditionalPropertiesInnerDTO additionalPropertiesDTO = new APIInfoAdditionalPropertiesInnerDTO();
    ...
    private static List<APIEndpointURLsInnerDTO> setEndpointURLsForAwsAPIs(ApiTypeWrapper model, String organization)
        ...
        List<APIEndpointURLsInnerDTO> endpointUrls = new ArrayList<>();
        APIEndpointURLsInnerURLsDTO APIEndpointURLsInnerURLsDTO = new APIEndpointURLsInnerURLsDTO();
        APIEndpointURLsInnerURLsDTO.setHttps(serverUrl);
        APIEndpointURLsInnerDTO.setUrLs(APIEndpointURLsInnerURLsDTO);
        endpointUrls.add(APIEndpointURLsInnerDTO);
        return endpointUrls;
    }

    public static List<APIEndpointURLsInnerDTO> fromAPIRevisionListToEndpointsList(APIDTO apidto, String organization)
            throws APIManagementException {
        ...
        for (APIRevisionDeployment revisionDeployment : revisionDeployments) {
            if (revisionDeployment.isDisplayOnDevportal()) {
                ...
                if (environment != null) {
                    APIEndpointURLsInnerDTO APIEndpointURLsInnerDTO = fromAPIRevisionToEndpoints(apidto, environment,
    ...
    private static APIEndpointURLsInnerDTO fromAPIRevisionToEndpoints(APIDTO apidto, Environment environment, String host,
        ...
        APIEndpointURLsInnerDTO.setEnvironmentDisplayName(environment.getDisplayName());
        ...
        if (!isWs) {
            if (apidto.getTransport().contains(APIConstants.HTTP_PROTOCOL)) {
                APIEndpointURLsInnerURLsDTO.setHttp(vHost.getHttpUrl() + context);
            }
            if (apidto.getTransport().contains(APIConstants.HTTPS_PROTOCOL)) {
                APIEndpointURLsInnerURLsDTO.setHttps(vHost.getHttpsUrl() + context);
            }
        }
        if (isWs || isGQLSubscription) {
            APIEndpointURLsInnerURLsDTO.setWs(vHost.getWsUrl() + context);
            APIEndpointURLsInnerURLsDTO.setWss(vHost.getWssUrl() + context);
        }
        APIEndpointURLsInnerDTO.setUrLs(APIEndpointURLsInnerURLsDTO);
        ...
        if (apidto.getIsDefaultVersion() != null && apidto.getIsDefaultVersion()) {
            String defaultContext = context.replaceAll("/" + apidto.getVersion() + "$", "");
            if (!isWs) {
                if (apidto.getTransport().contains(APIConstants.HTTP_PROTOCOL)) {
                    APIEndpointURLsInnerDefaultVersionURLsDTO.setHttp(vHost.getHttpUrl() + defaultContext);
                }
                if (apidto.getTransport().contains(APIConstants.HTTPS_PROTOCOL)) {
                    APIEndpointURLsInnerDefaultVersionURLsDTO.setHttps(vHost.getHttpsUrl() + defaultContext);
                }
            }
            if (isWs || isGQLSubscription) {
                APIEndpointURLsInnerDefaultVersionURLsDTO.setWs(vHost.getWsUrl() + defaultContext);
                APIEndpointURLsInnerDefaultVersionURLsDTO.setWss(vHost.getWssUrl() + defaultContext);
            }
        }
        APIEndpointURLsInnerDTO.setDefaultVersionURLs(APIEndpointURLsInnerDefaultVersionURLsDTO);

        return APIEndpointURLsInnerDTO;
    }
