...
import org.wso2.apk.discovery.api.Api;
import org.wso2.apk.discovery.api.Certificate;
import org.wso2.apk.discovery.api.Operation;
import org.wso2.apk.discovery.api.Resource;
import org.wso2.apk.discovery.api.Scopes;
import org.wso2.apk.discovery.api.SecurityList;
import org.wso2.apk.discovery.api.SecurityScheme;
...
//                        operation.getMethod()));
    ...
    private MockedApiConfig getMockedApiOperationConfig(
            org.wso2.apk.discovery.api.MockedApiConfig mockedApiConfig, String operationName) {
        MockedApiConfig configData = new MockedApiConfig();
        Map<String, MockedResponseConfig> responses = new HashMap<>();
        for (org.wso2.apk.discovery.api.MockedResponseConfig response : mockedApiConfig.getResponsesList()) {
            MockedResponseConfig responseData = new MockedResponseConfig();
            List<MockedHeaderConfig> headers = new ArrayList<>();
            for (org.wso2.apk.discovery.api.MockedHeaderConfig header : response.getHeadersList()) {
            ...
            for (org.wso2.apk.discovery.api.MockedContentConfig contentConfig : response.getContentList()) {
                MockedContentExamples mockedContentExamples = new MockedContentExamples();
                HashMap<String, String> exampleMap = new HashMap<>();
                for (org.wso2.apk.discovery.api.MockedContentExample exampleConfig :
