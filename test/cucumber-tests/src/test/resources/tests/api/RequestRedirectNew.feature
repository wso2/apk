Feature: Test HTTPRoute Filter Request Redirect functionality
    Scenario: Test request redirect functionality
        ...
        When I use the APK Conf file "artifacts/apk-confs/httproute-filters/request-redirect-filter-new.apk-conf"
    ...
    Scenario: Test request redirect functionality with API level redirect
        Given The system is ready
        And I have a valid subscription
        When I use the APK Conf file "artifacts/apk-confs/httproute-filters/api-level-redirect-new.apk-conf"
        And the definition file "artifacts/definitions/employees_api_new.json"
        And make the API deployment request
        Then the response status code should be 200
        Then I set headers
