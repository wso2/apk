Feature: Test HTTPRoute Filter Header Modifier functionality
  Scenario: Test request and response header modification functionality
    ...
    When I use the APK Conf file "artifacts/apk-confs/httproute-filters/header-modifier-filter-new.apk-conf"
  ...
  Scenario: Test request and response header modification functionality
    Given The system is ready
    And I have a valid subscription
    When I use the APK Conf file "artifacts/apk-confs/httproute-filters/api-level-header-new.apk-conf"
    And the definition file "artifacts/definitions/employees_api_new.json"
    And make the API deployment request
    Then the response status code should be 200
    Then I set headers
