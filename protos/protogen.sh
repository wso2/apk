...
printf "protoc java"
docker run -v `pwd`:/defs namely/protoc-all:$PROTOC_VERSION -l java -i . -i target/include/ -o target/gen/java -d wso2/
printf " - ${GREEN}${BOLD}done${NC}\n"
...
docker run -v `pwd`:/defs namely/protoc-all:$PROTOC_VERSION -l go --go-source-relative -i . -i target/include/ -o target/gen/go -d wso2/discovery/subscription/
printf " - ${GREEN}${BOLD}done${NC}\n"
...
docker run -v `pwd`:/defs namely/protoc-all:$PROTOC_VERSION -l go -i . -i target/include/ -o target/gen/go --go-package-map $import_map --go-source-relative -d wso2/discovery/service/apkmgt
