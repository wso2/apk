...
public class APIClient {
    ...
    private isolated function getDefinition(model:API api) returns json|commons:APKError {
        do {
            ...
            if definitionFileRef is string && definitionFileRef.length() > 0 {
                model:ConfigMap? configmapDefinition = check self.getDefinitionConfigmap(definitionFileRef, <string>api.metadata?.namespace);
    ...
    public isolated function deleteAPIById(string id, commons:Organization organization) returns http:Ok|commons:APKError {
        ...
        if (APIIDAvailable && string:length(id.toString()) > 0)
        {
            model:API? api = getAPI(id, organization);
            if api is model:API {
                http:Response|http:ClientError apiCRDeletionResponse = deleteAPICR(api.metadata.name, <string>api.metadata?.namespace);
                ...
                if definitionFileRef is string {
                    http:Response|http:ClientError apiDefinitionDeletionResponse = deleteConfigMap(definitionFileRef, <string>api.metadata?.namespace);
                ...
                _ = check self.deleteInternalAPI(api.metadata.name, <string>api.metadata?.namespace);
            } else {
                return e909001(id);
            }
        }
        ...
        do {
            ...
            if httpRouteListResponse is model:HttprouteList {
                foreach model:Httproute item in httpRouteListResponse.items {
                    http:Response|http:ClientError httprouteDeletionResponse = deleteHttpRoute(item.metadata.name, <string>item.metadata?.namespace);
        ...
        do {
            ...
            if backendPolicyListResponse is model:BackendList {
                foreach model:Backend item in backendPolicyListResponse.items {
                    http:Response|http:ClientError serviceDeletionResponse = deleteBackendPolicyCR(item.metadata.name, <string>item.metadata?.namespace);
        ...
        do {
            ...
            if authenticationCrListResponse is model:AuthenticationList {
                foreach model:Authentication item in authenticationCrListResponse.items {
                    http:Response|http:ClientError k8ServiceMappingDeletionResponse = deleteAuthenticationCR(item.metadata.name, <string>item.metadata?.namespace);
        ...
        do {
            ...
            if scopeCrListResponse is model:ScopeList {
                foreach model:Scope item in scopeCrListResponse.items {
                    http:Response|http:ClientError scopeCrDeletionResponse = deleteScopeCr(item.metadata.name, <string>item.metadata?.namespace);
    ...
    public isolated function createAPI(API api, string? definition, commons:Organization organization, string userName) returns commons:APKError|CreatedAPI {
        do {
            ...
            apiArtifact.namespace = getNameSpace(runtimeConfiguration.apiCreationNamespace);
    ...
    public isolated function generateK8sArtifacts(model:APIArtifact apiArtifact, APKConf apkConf, string? definition, commons:Organization organization, string uniqueId, string? userName) returns commons:APKError|error? {
        ...
        APKServiceInfo? serviceInfo = apkConf.serviceInfo;
        if serviceInfo is APKServiceInfo {
            Endpoint endpointConfig = {endpointURL: self.constructServiceURLFromServiceInfo(serviceInfo), endpointSecurity: serviceInfo.endpointSecurity};
            model:Backend backend = check self.createBackendService(apiArtifact, apkConf, (), PRODUCTION_TYPE, organization,
            endpointConfig);
    ...
    isolated function validateOperationPolicies(APIOperationPolicies? apiPolicies, APKOperations[] operations, commons:Organization organization) returns commons:APKError|() {
    ...
    isolated function validateRateLimit(APIRateLimit? apiRateLimit, APKOperations[] operations) returns commons:APKError|() {
        if (apiRateLimit == ()) {
            return ();
        } else {
            foreach APKOperations operation in operations {
    ...
    private isolated function createAndAddBackendServics(model:APIArtifact apiArtifact, APKConf apkConf, EndpointConfig endpointConfig, APKOperations? apiOperation, string? endpointType, commons:Organization organization) returns map<model:Endpoint>|commons:APKError|error {
    ...
    isolated function createAPIFromService(string serviceKey, API api, commons:Organization organization, string userName) returns CreatedAPI|commons:APKError {
        do {
            ...
            if serviceRetrieved is Service {
                PortMapping portMapping = self.retrievePort(serviceRetrieved);
                APKServiceInfo? serviceInfo = apkConf.serviceInfo;
                if serviceInfo is APKServiceInfo {
                    APKServiceInfo clonedServiceInfo = serviceInfo.clone();
                    clonedServiceInfo.port = portMapping.port;
                    clonedServiceInfo.protocol = portMapping.protocol;
                    clonedServiceInfo.name = serviceRetrieved.name;
                    clonedServiceInfo.namespace = serviceRetrieved.namespace;
                    apkConf.serviceInfo = clonedServiceInfo;
                } else {
                    APKServiceInfo clonedServiceInfo = {
                ...
                model:APIArtifact apiArtifact = {uniqueId: uniqueId};
                check self.generateK8sArtifacts(apiArtifact, apkConf, (), organization, uniqueId, userName);
                apiArtifact.namespace = getNameSpace(runtimeConfiguration.apiCreationNamespace);
    ...
    private isolated function constructServiceURLFromServiceInfo(APKServiceInfo 'apkServiceInfo) returns string {
        return <string>apkServiceInfo.protocol + "://" + string:'join(".", <string>apkServiceInfo.name, <string>apkServiceInfo.namespace, "svc.cluster.local") + ":" + apkServiceInfo.port.toString();
    }
    ...
    private isolated function deployAPIToK8s(model:APIArtifact apiArtifact, commons:Organization organization) returns commons:APKError|model:API {
        do {
            ...
            if definition is model:ConfigMap {
                definition.metadata.namespace = apiArtifact.namespace;
                _ = check self.deployConfigMap(definition, <string>apiArtifact?.namespace);
            }
            model:API? api = apiArtifact.api;
            if api is model:API {
                api.metadata.namespace = apiArtifact.namespace;
                ...
                check self.deleteSecrets(api, organization);
                check self.deleteInternalAPI(api.metadata.name, <string>api.metadata?.namespace);
            }
            ...
            check self.deploySecrets(apiArtifact);
    ...
    private isolated function deploySecrets(model:APIArtifact apiArtifact) returns error? {
        foreach model:K8sSecret secret in apiArtifact.secrets {
            secret.metadata.namespace = apiArtifact.namespace;
            http:Response deploySecretResult = check createK8sSecret(secret);
            if deploySecretResult.statusCode == http:STATUS_CREATED {
                log:printDebug("Deployed Secret Successfully" + secret.toString());
            } else {
                json responsePayLoad = check deploySecretResult.getJsonPayload();
                model:Status statusResponse = check responsePayLoad.cloneWithType(model:Status);
                check self.handleK8sTimeout(statusResponse);
            }
        }
    ...
    private isolated function deployEndpointCertificates(model:APIArtifact apiArtifact) returns error? {
        ...
        foreach model:ConfigMap endpointCertificate in endpointCertificates {
            endpointCertificate.metadata.namespace = apiArtifact.namespace;
            _ = check self.deployConfigMap(endpointCertificate, <string>apiArtifact?.namespace);
        }
    }
    private isolated function deleteEndpointCertificates(model:API api, commons:Organization organization) returns commons:APKError? {
        do {
            ...
            foreach model:ConfigMap endpointCertificate in endpointCertificates {
                http:Response deleteEndpointCertificateResult = check deleteConfigMap(endpointCertificate.metadata.name, <string>endpointCertificate.metadata?.namespace);
    ...
    private isolated function deployScopeCrs(model:APIArtifact apiArtifact) returns error? {
        foreach model:Scope scope in apiArtifact.scopes {
            scope.metadata.namespace = apiArtifact.namespace;
            http:Response deployScopeResult = check deployScopeCR(scope);
    ...
    private isolated function deployK8sAPICr(model:APIArtifact apiArtifact) returns model:API|commons:APKError|error {
        ...
        if k8sAPI is model:API {
            model:API? k8sAPIByNameAndNamespace = check getK8sAPIByNameAndNamespace(k8sAPI.metadata.name, <string>k8sAPI.metadata?.namespace);
            if k8sAPIByNameAndNamespace is model:API {
                ...
                http:Response deployAPICRResult = check updateAPICR(k8sAPI);
                ...
                http:Response deployAPICRResult = check deployAPICR(k8sAPI);
    ...
    private isolated function deployRuntimeAPI(model:APIArtifact apiArtifact) returns error? {
        ...
        if runtimeapi is model:RuntimeAPI {
            runtimeapi.metadata.namespace = apiArtifact.namespace;
            log:printInfo("inside deploy Deploying RuntimeAPI " + runtimeapi.toJsonString());
            http:Response deployRuntimeAPICRResult = check createInternalAPI(runtimeapi);
    ...
    private isolated function deployHttpRoutes(model:Httproute[] httproutes, string namespace) returns error? {
        foreach model:Httproute httpRoute in httproutes {
            if httpRoute.spec.rules.length() > 0 {
                ...
                http:Response deployHttpRouteResult = check deployHttpRoute(httpRoute);
    ...
    private isolated function deployServiceMappings(model:APIArtifact apiArtifact) returns error? {
        foreach model:K8sServiceMapping k8sServiceMapping in apiArtifact.serviceMapping {
            k8sServiceMapping.metadata.namespace = apiArtifact.namespace;
            http:Response deployServiceMappingCRResult = check deployServiceMappingCR(k8sServiceMapping);
    ...
    private isolated function deployAuthneticationCRs(model:APIArtifact apiArtifact) returns error? {
        ...
        foreach string authenticationCrName in keys {
            ...
            authenticationCr.metadata.namespace = apiArtifact.namespace;
            log:printDebug("Authentication CR:" + authenticationCr.toString());
            http:Response authenticationCrDeployResponse = check deployAuthenticationCR(authenticationCr);
    ...
    private isolated function deployBackendServices(model:APIArtifact apiArtifact) returns error? {
        foreach model:Backend backendService in apiArtifact.backendServices {
            backendService.metadata.namespace = apiArtifact.namespace;
            http:Response deployServiceResult = check deployBackendCR(backendService);
    ...
    private isolated function deployConfigMap(model:ConfigMap definition, string namespace) returns model:ConfigMap|commons:APKError|error {
        definition.metadata.namespace = namespace;
        http:Response configMapRetrieved = check getConfigMapValueFromNameAndNamespace(definition.metadata.name, <string>definition.metadata.namespace);
        if configMapRetrieved.statusCode == 404 {
            http:Response deployConfigMapResult = check deployConfigMap(definition);
            ...
            http:Response deployConfigMapResult = check updateConfigMap(definition);
    ...
    private isolated function updateConfigMap(model:ConfigMap configMap) returns model:ConfigMap|commons:APKError|error {
        ...
        if configMapRetrieved.statusCode == 200 {
            http:Response deployConfigMapResult = check updateConfigMap(configMap);
            if deployConfigMapResult.statusCode == http:STATUS_OK {
    ...
    private isolated function deployRateLimitPolicyCRs(model:APIArtifact apiArtifact) returns error? {
        foreach model:RateLimitPolicy rateLimitPolicy in apiArtifact.rateLimitPolicies {
            rateLimitPolicy.metadata.namespace = apiArtifact.namespace;
            http:Response deployRateLimitPolicyResult = check deployRateLimitPolicyCR(rateLimitPolicy);
    ...
    private isolated function createK8sSecret(string uniqueSecretName, string username, string password) returns model:K8sSecret {
    ...
    private isolated function prepareSecret(APKConf apkConf, string uniqueSecretName, model:K8sSecret prevK8sSecret, commons:Organization organization) returns model:K8sSecret {
        return {
            metadata: {
                name: uniqueSecretName,
                labels: self.getLabels(apkConf, organization)
            },
            data: prevK8sSecret.data,
            stringData: prevK8sSecret.stringData
        };
    }
    ...
    private isolated function updateK8sSecret(string uniqueSecretName, string username, string password) returns error? {
        ...
        http:Response updateK8sResult = check updateK8sSecret(uniqueSecretName, k8sSecret);
    ...
    private isolated function deployAPIPolicyCRs(model:APIArtifact apiArtifact) returns error? {
        foreach model:APIPolicy apiPolicy in apiArtifact.apiPolicies {
            ...
            http:Response deployAPIPolicyResult = check deployAPIPolicyCR(apiPolicy);
    ...
    private isolated function deployInterceptorServiceCRs(model:APIArtifact apiArtifact) returns error? {
        foreach model:InterceptorService interceptorService in apiArtifact.interceptorServices {
            interceptorService.metadata.namespace = apiArtifact.namespace;
            http:Response deployAPIPolicyResult = check deployInterceptorServiceCR(interceptorService);
            if deployAPIPolicyResult.statusCode == http:STATUS_CREATED {
    ...
    private isolated function deleteSecrets(model:API api, commons:Organization organization) returns commons:APKError? {
        do {
            model:K8sSecretList|http:ClientError k8sSecretListResponse = check getSecretsForAPI(api.spec.apiDisplayName, api.spec.apiVersion, <string>api.metadata?.namespace, organization);
            if k8sSecretListResponse is model:K8sSecretList {
                foreach model:K8sSecret item in k8sSecretListResponse.items {
                    http:Response|http:ClientError secretDeletionResponse = deleteK8sSecret(item.metadata.name, <string>item.metadata?.namespace);
                    if secretDeletionResponse is http:Response {
                        if secretDeletionResponse.statusCode != http:STATUS_OK {
                            json responsePayLoad = check secretDeletionResponse.getJsonPayload();
                            model:Status statusResponse = check responsePayLoad.cloneWithType(model:Status);
                            check self.handleK8sTimeout(statusResponse);
                        }
                    } else {
                        log:printError("Error occured while deleting K8s Secrets");
                    }
                }
                return;
            }
        } on fail var e {
            log:printError("Error occured deleting K8s secrets", e);
            return error("Error occured deleting K8s secrets", message = "Internal Server Error", code = 909000, description = "Internal Server Error", statusCode = 500);
        }
    }
    ...
    private isolated function generateAPIPolicyAndBackendCR(model:APIArtifact apiArtifact, APKConf apkConf, APKOperations? operations, APIOperationPolicies? policies, commons:Organization organization, string targetRefName) returns model:APIPolicy? {
    ...
    private isolated function generateHttpRouteRule(model:APIArtifact apiArtifact, APKConf apkConf, model:Endpoint? endpoint, APKOperations operation, string endpointType, commons:Organization organization) returns model:HTTPRouteRule|()|commons:APKError {
        do {
            EndpointConfig? endpointConfig = operation.endpointConfig;
    ...
    isolated function generateAndSetK8sServiceMapping(model:APIArtifact apiArtifact, APKConf apkConf, Service serviceEntry, commons:Organization organization) {
        model:API? k8sAPI = apiArtifact.api;
        if k8sAPI is model:API {
    ...
    isolated function deleteServiceMappings(model:API api, commons:Organization organization) returns commons:APKError? {
        do {
            ...
            model:ServiceMappingList|http:ClientError k8sServiceMapingsDeletionResponse = check getK8sServiceMapingsForAPI(api.spec.apiDisplayName, api.spec.apiVersion, <string>api.metadata?.namespace, organization);
            ...
            foreach string key in keys {
                ...
                http:Response|http:ClientError k8ServiceMappingDeletionResponse = deleteK8ServiceMapping(serviceMapping.metadata.name, <string>serviceMapping.metadata.namespace);
                if k8ServiceMappingDeletionResponse is http:Response {
                    if k8ServiceMappingDeletionResponse.statusCode != http:STATUS_OK {
                        json responsePayLoad = check k8ServiceMappingDeletionResponse.getJsonPayload();
                        model:Status statusResponse = check responsePayLoad.cloneWithType(model:Status);
    ...
    isolated function createBackendService(model:APIArtifact apiArtifact, APKConf apkConf, APKOperations? apiOperation, string endpointType, commons:Organization organization, Endpoint endpointConfig) returns commons:APKError|model:Backend|error {
        ...
        if endpointSecurity is EndpointSecurity {
            if endpointSecurity?.enable ?: false {
                ...
                if endpointSecurity?.securityType == ENDPOINT_SECURITY_TYPE_BASIC_CASE {
                    // validate data
                    record {}? securityProperties = endpointSecurity.securityProperties;
                    if securityProperties is record {} {
                        string username = securityProperties.hasKey(ENDPOINT_SECURITY_USERNAME) ? <string>securityProperties.get(ENDPOINT_SECURITY_USERNAME) : "";
                        string password = securityProperties.hasKey(ENDPOINT_SECURITY_PASSWORD) ? <string>securityProperties.get(ENDPOINT_SECURITY_PASSWORD) : "";
                        if username == "" || password == "" {
                            return e909051();
                        }
                        model:K8sSecret k8sSecret = self.createK8sSecret(uniqueSecretName, username, password);
                        apiArtifact.secrets[k8sSecret.metadata.name] = k8sSecret;
                    }
                }
                securityConfig = {
        ...
        if configMapName is string && backendService.spec.protocol == "https" {
            backendService.spec.tls = {
                configMapRef: {
                    key: endpointConfig.certificate ?: CERTIFICATE_KEY_CONFIG_MAP,
                    name: configMapName
                }
            };
        }
    ...
    public isolated function generateRateLimitPolicyCR(APKConf apkConf, APIRateLimit? rateLimit, string targetRefName, APKOperations? operation, commons:Organization organization) returns model:RateLimitPolicy? {
    ...
    isolated function retrieveAPIPolicyDetails(model:APIArtifact apiArtifact, APKConf apkConf, APKOperations? operations, commons:Organization organization, OperationPolicy[]? policies, string flow) returns model:InterceptorReference? {
    ...
    private isolated function prepareBackendRef(model:HTTPBackendRef backendRef, model:APIArtifact apiArtifact, Service? serviceEntry, APKConf oldAPKConf, APKConf newAPKConf, string endpointType, commons:Organization organization) returns [string, string]?|error {
        if apiArtifact.serviceMapping.length() >= 1 {
            if serviceEntry is Service {
                ...
                model:Backend backendService = check self.createBackendService(apiArtifact, newAPKConf, (), PRODUCTION_TYPE, organization, endpointConfig);
                backendRef.name = backendService.metadata.name;
                backendRef.namespace = <string>backendService.metadata.namespace;
                ...
                self.generateAndSetK8sServiceMapping(apiArtifact, newAPKConf, serviceEntry, organization);
    ...
    private isolated function prepareBackendRefForAPIArtifact(model:HTTPBackendRef backendRef, model:APIArtifact apiArtifact, APKConf newAPKConf, string endpointType, commons:Organization organization) returns [string, string]?|error {
        ...
        if backendServices.hasKey(oldBackendRefName) {
            ...
            _ = check self.prepareBackendSecurityForEndpoints(apiArtifact, 'service, endpointType, newAPKConf, organization);
    ...
    private isolated function prepareBackendSecurityForEndpoints(model:APIArtifact apiArtifact, model:Backend 'service, string endpointType, APKConf newAPKConf, commons:Organization organization) returns error? {
        if 'service.spec.security is model:SecurityConfig {
            model:SecurityConfig? security = 'service.spec.security;
            if security is model:SecurityConfig {
                if security.'type == ENDPOINT_SECURITY_TYPE_BASIC_CASE {
                    model:BasicSecurityConfig? basicSecurity = security.basic;
                    if basicSecurity is model:BasicSecurityConfig {
                        model:SecretRefConfig secretRef = basicSecurity.secretRef;
                        if (apiArtifact.secrets.hasKey(secretRef.name)) {
                            model:K8sSecret prevSecret = apiArtifact.secrets.get(secretRef.name);
                            string newSecretRefName = getBackendSecurityUid(endpointType, organization);
                            model:K8sSecret newSecret = self.prepareSecret(newAPKConf, newSecretRefName, prevSecret, organization);
                            apiArtifact.secrets['newSecret.metadata.name] = newSecret;
                            _ = apiArtifact.secrets.removeIfHasKey(secretRef.name);
                            secretRef.name = 'newSecret.metadata.name;
                        }
                    }
                }
            }
    ...
    private isolated function getApiArtifact(API api, commons:Organization organization) returns model:APIArtifact|error {
        ...
        if k8sapi is model:API {
            ...
            if definitionFileRef is string {
                model:ConfigMap|error? definitionConfigmap = check self.getDefinitionConfigmap(definitionFileRef, <string>k8sapi.metadata?.namespace);
            ...
            if httpProdRouteRefs is json[] && httpProdRouteRefs.length() > 0 {
                foreach json prodHTTPRouteRef in httpProdRouteRefs {
                    model:Httproute httpRoute = check getHttpRoute(prodHTTPRouteRef.toString(), <string>k8sapi.metadata?.namespace);
            ...
            if httpSandRouteRefs is json[] && httpSandRouteRefs.length() > 0 {
                foreach json sandHTTPRouteRef in httpSandRouteRefs {
                    model:Httproute httpRoute = check getHttpRoute(sandHTTPRouteRef.toString(), <string>k8sapi.metadata?.namespace);
                    apiArtifact.sandboxRoute.push(self.sanitizeHttpRoute(httpRoute));
                }
            }
            model:ServiceMappingList k8sServiceMapingsForAPI = check getK8sServiceMapingsForAPI(api.name, api.'version, <string>k8sapi.metadata?.namespace, organization);
            ...
            model:AuthenticationList authenticationCrsForAPI = check getAuthenticationCrsForAPI(api.name, api.'version, <string>k8sapi.metadata?.namespace, organization);
            ...
            model:BackendList backendList = check getBackendPolicyCRsForAPI(api.name, api.'version, <string>k8sapi.metadata?.namespace, organization);
            ...
            model:RuntimeAPI|http:ClientError internalAPI = getInternalAPI(k8sapi.metadata.name, <string>k8sapi.metadata?.namespace);
            if internalAPI is model:RuntimeAPI {
            ...
            model:ScopeList scopeList = check getScopeCrsForAPI(k8sapi.spec.apiDisplayName, k8sapi.spec.apiVersion, <string>k8sapi.metadata?.namespace, organization);
            foreach model:Scope scope in scopeList.items {
                apiArtifact.scopes[scope.metadata.name] = self.sanitizeScopeCR(scope);
            }
            model:RateLimitPolicyList rateLimitPolicyList = check getRateLimitPolicyCRsForAPI(k8sapi.spec.apiDisplayName, k8sapi.spec.apiVersion, <string>k8sapi.metadata?.namespace, organization);
            ...
            model:APIPolicyList apiPolicyList = check getAPIPolicyCRsForAPI(k8sapi.spec.apiDisplayName, k8sapi.spec.apiVersion, <string>k8sapi.metadata?.namespace, organization);
            ...
            model:K8sSecretList k8sSecretListResponse = check getSecretsForAPI(k8sapi.spec.apiDisplayName, k8sapi.spec.apiVersion, <string>k8sapi.metadata?.namespace, organization);
            foreach model:K8sSecret k8sSecret in k8sSecretListResponse.items {
                apiArtifact.secrets[k8sSecret.metadata.name] = self.sanitizeK8sSecret(k8sSecret);
            }
            return apiArtifact;
        } else {
    ...
    private isolated function sanitizeScopeCR(model:Scope scope) returns model:Scope {
        model:Scope sanitizedScope = {
            metadata: {name: scope.metadata.name, labels: scope.metadata.labels},
            spec: scope.spec
        };
    ...
    private isolated function sanitizeAPIPolicyCR(model:APIPolicy apiPolicy) returns model:APIPolicy {
        model:APIPolicy sanitizedAPIPolicy = {
            metadata: {name: apiPolicy.metadata.name, labels: apiPolicy.metadata.labels},
            spec: apiPolicy.spec
        };
    ...
    private isolated function sanitizeRuntimeAPI(model:RuntimeAPI runtimeAPI) returns model:RuntimeAPI {
        model:RuntimeAPI sanitizedAPI = {
            metadata: {name: runtimeAPI.metadata.name, labels: runtimeAPI.metadata.labels},
    ...
    private isolated function sanitizeK8sSecret(model:K8sSecret secret) returns model:K8sSecret {
        return {
            metadata: {
    ...
    private isolated function zipAPIArtifact(string apiId, model:APIArtifact apiArtifact) returns [string, string]|error {
        ...
        foreach model:K8sSecret secret in apiArtifact.secrets {
            _ = check self.convertAndStoreYamlFile(secret.toJsonString(), secret.metadata.name, zipDir, "secrets");
        }
        model:RuntimeAPI? runtimeAPI = apiArtifact.runtimeAPI;
        if runtimeAPI is model:RuntimeAPI {
        ...
        do {
            ...
            if api is API {
                ...
                if serviceInfo is API_serviceInfo {
                    ...
                    if serviceEntry is Service {
                        PortMapping portMapping = self.retrievePort(serviceEntry);
                        APKServiceInfo? apkServiceInfo = apkConf.serviceInfo;
                        if apkServiceInfo is APKServiceInfo {
                            APKServiceInfo clonedServiceInfo = apkServiceInfo.clone();
                            clonedServiceInfo.port = portMapping.port;
                            clonedServiceInfo.protocol = portMapping.protocol;
                            clonedServiceInfo.name = serviceEntry.name;
                            clonedServiceInfo.namespace = serviceEntry.namespace;
                            apkConf.serviceInfo = clonedServiceInfo;
                        } else {
                            APKServiceInfo clonedServiceInfo = {
                                name: serviceEntry.name,
                ...
                check self.generateK8sArtifacts(apiArtifact, apkConf, (), organization, uniqueId, userName);
                if serviceEntry is Service {
                    self.generateAndSetK8sServiceMapping(apiArtifact, apkConf, serviceEntry, organization);
        ...
        do {
            ...
            if api is model:API {
                ...
                if (validateCertificateExpiryResult[1]) {
                    ...
                    model:ConfigMap deployedConfigMap = check self.deployConfigMap(certificateConfigMapEntry, getNameSpace(runtimeConfiguration.apiCreationNamespace));
