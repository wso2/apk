...
import org.wso2.apk.runtime.queryanalysis.CustomComplexityDetails;
import org.wso2.apk.runtime.queryanalysis.GraphqlComplexityInfo;
import org.wso2.apk.runtime.queryanalysis.GraphqlSchemaType;
...
public class GraphQLSchemaDefinition {
        ...
        if (swaggerDef != null) {
            ...
            if (operationAuthSchemeMap.size() > 0) {
                ...
                String base64EncodedPolicyDefinition = Base64.getUrlEncoder().withoutPadding().
                        encodeToString(jsonPolicyDefinition.getAsString().getBytes(Charset.defaultCharset()));
    ...
    public JsonObject policyDefinitionToJson(GraphqlComplexityInfo graphqlComplexityInfo) {
        ...
        JsonObject customComplexityObject = new JsonObject();
        for (HashMap.Entry<String, HashMap<String, Integer>> entry : customComplexityMap.entrySet()) {
            ...
            JsonObject fieldValueObject = new JsonObject();
            for (HashMap.Entry<String, Integer> subEntry : fieldValueMap.entrySet()) {
                ...
                fieldValueObject.addProperty(field, complexityValue);
            }
            customComplexityObject.add(type, fieldValueObject);
        }

        policyDefinition.add(APIConstants.QUERY_ANALYSIS_COMPLEXITY, customComplexityObject);
