 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function showUninstallConflictsNotification(
  ...
  if (conflictingExts.length === 1) {
    ...
    conflictMsg = `The ${extNames.join(
      ", "
    )} extensions are incompatible with VSCode-YAML. Please uninstall them.`;
  }
    ...
    window
      ...
      .then((clickedMsg) => {
        if (clickedMsg === uninstallMsg) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          if (
            conflictingExts.findIndex((ext: any) => ext.id === azureDeploy) !==
            -1
          ) {
