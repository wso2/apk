...
import config_generator_service.model;
import config_generator_service.org.wso2.apk.config.model as runtimeModels;
import ballerina/regex;
import config_generator_service.org.wso2.apk.config as runtimeUtil;
import ballerina/mime;
import ballerina/lang.value;
import config_generator_service.org.wso2.apk.config.api as runtimeapi;
...
import config_generator_service.java.util as utilapis;
import ballerina/jballerina.java;

public class APIClient {
    public isolated function getGeneratedAPKConf(http:Request request) returns commons:APKError|OkAnydata|BadRequestError {
        do {
            ...
            if validateAndRetrieveDefinitionResult is runtimeapi:APIDefinitionValidationResponse {
                if validateAndRetrieveDefinitionResult.isValid() {
                    ...
                    APKConf generatedAPKConf = self.fromAPIModelToAPKConf(apiFromDefinition);
    ...
    private isolated function fromAPIModelToAPKConf(runtimeModels:API api) returns APKConf {
    ...
    public isolated function generateK8sArtifacts(model:APIArtifact apiArtifact, APKConf apkConf, string? definition, string organization) returns commons:APKError|error? {
        ...
        if (policyData is APKOperationPolicy[]) {
            foreach APKOperationPolicy policy in policyData {
                ...
                if (policyParameters is record {}) {
                    ...
                    foreach model:MediationPolicy & readonly mediationPolicy in avilableMediationPolicyList {
    ...
    private isolated function generateAndRetrieveParentRefs(APKConf apkConf, string uniqueId) returns model:ParentReference[] {
        string gatewayName = configuratorConfiguration.gatewayConfiguration.name;
        string listenerName = configuratorConfiguration.gatewayConfiguration.listenerName;
    ...
    public isolated function getGeneratedK8sResources(http:Request request) returns http:Response|BadRequestError|InternalServerErrorError|commons:APKError {
        ...
        do {
            ...
            model:APIArtifact apiArtifact = check self.prepareArtifact(body);
    ...
    private isolated function prepareArtifact(GenerateK8sResourcesBody artifactRequest) returns commons:APKError|model:APIArtifact|error {
        record {|byte[] fileContent; string fileName; anydata...;|}? apkConfiguration = artifactRequest.apkConfiguration;
        record {|byte[] fileContent; string fileName; anydata...;|}? definitionFile = artifactRequest.definitionFile;
        string? apiType = artifactRequest.apiType;
        if apkConfiguration is () && definitionFile is () && apiType is () {
        ...
        do {
            ...
            string uniqueId = self.getUniqueIdForAPI(<string>apkConf?.name, <string>apkConf?.'version, organization);
            model:APIArtifact apiArtifact = {uniqueId: uniqueId};
            check self.generateK8sArtifacts(apiArtifact, apkConf, apiDefinition, organization);
            return apiArtifact;
        } on fail var e {
