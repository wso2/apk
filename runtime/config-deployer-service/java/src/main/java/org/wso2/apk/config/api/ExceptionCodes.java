...
/**
 * This enum class holds error codes that we need to pass to upper level. For
 * example, to the UI.
...
public enum ExceptionCodes implements ErrorHandler {
        ...
        API_CONTEXT_ALREADY_EXISTS(900251, "The API context already exists.", 409,
                        "An API with context '%s' already exists"),
        API_VERSION_ALREADY_EXISTS(900252, "The API version already exists.", 409,
                        "An API with version '%s' already exists for API '%s'"),
        API_EMPTY_PASSWORD_FOR_SECURED_ENDPOINT(900253,
                        "Empty password is given for endpointSecurity when creating API: %s",
                        400, "An empty password is given for endpointSecurity when creating API: %s"),

        API_PRODUCT_CONTEXT_ALREADY_EXISTS(900275, "The API Product context already exists.", 409,
                        "An API Product with context '%s' already exists"),
        ...
        APPLICATION_DELETE_FAILED(900309, "Error has occurred. Could not delete the application", 500,
                        "Error has occurred. Could not delete the application '%s'"),
        SUBSCRIPTION_NOT_FOUND(900310, "Subscription not found", 404, "Couldn't retrieve Subscriptions for API"),
        UPDATE_STATE_CHANGE(900311, "API fields have state changes", 400,
                        "Couldn't Update as API have changes can't be done"),
        DOCUMENT_ALREADY_EXISTS(900312, "Document already exists", 409, "Document already exists"),
        COULD_NOT_UPDATE_API(900313, "Error has occurred. Could not update the API", 500,
                        "Error has occurred. Could not "
                                        + "update the API"),
        ...
        DOCUMENT_INVALID_SOURCE_TYPE(900319, "Invalid document source type", 500,
                        "Source type of the document '%s' is invalid"),
        UNSUPPORTED_DOC_EXTENSION(900367, "Document file type not supported", 400,
                        "Unsupported extension type of document file"),
        ...
        RATING_VALUE_INVALID(900325, "Rating value invalid", 400,
                        "Provided rating value does not fall in between min max "
                                        + "values"),
        DOCUMENT_INVALID_VISIBILITY(900326, "Invalid document visibility type", 500,
                        "Visibility type of the document '%s' is invalid"),
        ...
        APPLICATION_KEY_MAPPING_NOT_FOUND(900331, "Application Key mapping not found", 404,
                        "Application Key mapping not " +
                                        "found"),
        ...
        APPLICATION_EXPORT_ERROR(900338, "Application Export Error", 500,
                        "Error while exporting the given Application"),
        APPLICATION_IMPORT_ERROR(900339, "Application Import Error", 500,
                        "Error while importing the given Application"),
        ...
        API_PRODUCT_RESOURCE_ENDPOINT_UNDEFINED(900342,
                        ...
                        409,
                        "Cannot create API Product %s, due to resources with undefined endpoints in their parent APIs %s"),
        API_PRODUCT_WITH_UNSUPPORTED_LIFECYCLE_API(900343,
                        ...
                        409,
                        "Cannot create API Product, due to resources parent API being in an unsupported Life Cycle state: %s"),
        API_PRODUCT_USED_RESOURCES(900344,
                        "Cannot remove the resource paths because they are used by one or more API Products",
                        409,
                        "Cannot update API: %s:%s, due to the resources to remove are used by one or more API Products"),
        ...
        EXISTING_API_REVISION_DEPLOYMENT_FOUND(900348, "Can not delete API Revision ", 400,
                        "Couldn't delete API revision since API revision is currently deployed to a gateway. " +
                                        "You need to undeploy the API Revision from the gateway before attempting deleting API Revision: %s "),
        EXISTING_API_REVISION_FOUND(900349, "Can not create API Revision ", 400,
                        "API revision already exists with id: %s "),
        API_REVISION_UUID_NOT_FOUND(900350, "Can not create API Revision ", 400,
                        "Failed to retrieve revision uuid from revision registry artifact"),
        MAXIMUM_REVISIONS_REACHED(900351, "Can not create API Revision ", 400,
                        "Maximum number of revisions per API has reached." +
                                        "Need to remove any revision to create a new Revision for API with API UUID: %s"),
        ERROR_CREATING_API_REVISION(900352, "Can not create API Revision ", 400,
                        "Failed to create API revision registry artifacts: %s "),
        ERROR_DELETING_API_REVISION(900353, "Can not delete API Revision ", 400,
                        "Failed to delete API revision registry artifacts: %s "),
        ERROR_RESTORING_API_REVISION(900354, "Can not restore API Revision ", 400,
                        "Failed to restore API revision registry artifacts: %s "),
        ...
        API_OR_API_PRODUCT_NOT_FOUND(900359, "API or API Product Not Found", 404,
                        "Requested API or API Product with id '%s' not found"),
        API_PRODUCT_NOT_FOUND(900360, "API Product Not Found", 404, "Requested API Product with id '%s' not found"),
        SUB_ORGANIZATION_NOT_IDENTIFIED(900361, "User's Organization Not Identified", 403,
                        "User's Organization is not identified"),
        ERROR_RETRIEVING_CATEGORY(900362, "Cannot retrieve categories", 500,
                        "Error while retrieving categories for organization '%s'"),
        PERSISTENCE_ERROR(900363, "Error occurred in registry transaction", 500, "'%s'"),
        NO_VIEW_UPDATE_PERMISSIONS(900365, "Insufficient permission to view or update the API", 403,
                        "Insufficient permission to view or update the API"),
        API_DELETE_FAILED_SUBSCRIPTIONS(900366, "Failed to delete the API", 409,
                        "Cannot remove the API as active subscriptions exist"),
        CATEGORY_NAME_CONTAINS_SPECIAL_CHARS(900368, "Name field contains special characters", 400,
                        "API Category name contains special characters"),
        CATEGORY_NAME_TOO_LONG(900369, "API Category name is too long", 400,
                        "API Category name exceeds 255 characters"),
        CATEGORY_ALREADY_EXISTS(900370, "API Category name already exists", 409,
        ...
        ERROR_CHANGING_APP_OWNER(900373, "Failed to change the application owner", 500,
                        "Error while changing the application owner"),

        USER_SUBSCRIPTION_EXISTS_CHECK_FAILED(900374, "Failed to check if user subscribed to API", 500,
                        "Failed to check if user '%s' with an application '%s' is subscribed to API %s"),

        USER_ACCESSIBLE_APPLICATION_CHECK_FAILED(900375, "Failed to check if user subscribed to API", 500,
                        "Error occurred while checking whether the application '%s' is accessible to user '%s'"),
        ...
        UNSUPPORTED_LIFECYCLE_ACTION(900381, "Unsupported state change action", 400,
                        "Lifecycle state change action %s is not allowed"),
        LIFECYCLE_STATE_INFORMATION_NOT_FOUND(900382, "Lifecycle state information not found", 500,
                        "Lifecycle state change information for %s with %s cannot be found"),
        ...
        ENDPOINT_CONFIG_PARSE_FAILED(900455, "Endpoint config parsing failed", 500,
                        "Error occurred while parsing endpoint config json"),
        ...
        INVALID_GATEWAY_ENVIRONMENT(900504, "Invalid Gateway Environment", 400,
        ...
        IDP_ADDING_FAILED(900611, "Unable to add the identity provider", 400,
                        "Error while adding the identity provider"),
        ...
        ANON_USER_ACTION(900616, "Operation not permitted", 401,
                        "Attempt to execute privileged operation as the anonymous user"),
        ...
        API_NOT_SOAPTOREST(900687, "Provided API is not a SOAP to REST converted API", 400,
                        "Provided API is not a SOAP to REST converted API"),
        ...
        OPENAPI_PARSE_EXCEPTION_WITH_CUSTOM_MESSAGE(OPENAPI_PARSE_EXCEPTION.getErrorCode(),
                        OPENAPI_PARSE_EXCEPTION.getErrorMessage(), OPENAPI_PARSE_EXCEPTION.getHttpStatusCode(), "'%s'"),
        ...
        ERROR_REMOVING_EXAMPLES(900764, "Internal Error While Processing Swagger Definition", 500,
                        "Couldn't remove one or more examples from the swagger definition"),
        MOCK_HTTP_METHOD_MISSING(900765, "Could not find HTTP methods", 400,
                        "Cannot find the HTTP method for the API Resource Mediation Policy"),
        SWAGGER_ARCHIVE_MISSING(900766, "Could not find an archive in the given ZIP file", 500,
                        "Could not find an archive in the given ZIP file"),

        // AsyncApi related error codes
        ASYNCAPI_URL_MALFORMED(900756, "AsyncAPI specification retrieval from URL failed", 400,
                        "Exception occurred while retrieving the AsyncAPI Specification from URL"),
        ASYNCAPI_URL_NO_200(900757, "AsyncAPI specification retrieval from URL failed", 400,
                        "Response didn't return a 200 OK status"),

        ERROR_READING_ASYNCAPI_SPECIFICATION(900765, "AsyncAPI specification read error", 500,
                        "Exception occurred while reading the AsyncAPI Specification file"),
        ...
        INVALID_PARAMETERS_PROVIDED(900708, "Invalid parameter(s) provided", 400,
                        "Bad Request. Mandatory parameters are invalid/missing"),
        ...
        API_NOT_GRPC(900800, "This API is not a GRPC API", 400, "This API is not a GRPC API"),
        PROTO_DEFINITION_PARSE_EXCEPTION(900754, "Error while parsing the proto definition", 400,
                        "Error while parsing the proto definition"),
        ...
        APPLICATION_TOKEN_GENERATION_FAILED(900957, "Keymanagement Error", 500,
                        " Error while generating the application" +
                                        "access token."),
        APPLICATION_CONSUMER_KEY_NOT_FOUND(900958, "Keymanagement Error", 403,
                        "Requested consumer key with application '%s' not found"),
        ...
        ADVANCED_POLICY_EXISTS(902900, "Advanced policy already exists", 409,
                        "Advanced Policy with name '%s' already exists"),
        APPLICATION_POLICY_EXISTS(902901, "Application policy already exists", 409,
                        "Application Policy with name '%s' already exists"),
        SUBSCRIPTION_POLICY_EXISTS(902902, "Subscription policy already exists", 409,
                        "Subscription Policy with name '%s' already exists"),
        ...
        SUBSCRIPTION_POLICY_GET_ALL_FAILED(902907, "Error while retrieving Subscription level policies", 500,
                        "Server Error Occurred"),
        SUBSCRIPTION_POLICY_ADD_FAILED(902908, "Error while adding Subscription level policies", 500,
                        "Server Error Occurred"),
        SUBSCRIPTION_POLICY_GET_FAILED(902909, "Error while retrieving Subscription level policy : '%s'", 500,
                        "Server Error Occurred"),
        BAD_POLICY_OBJECT(902010, "Policy object doesn't contain mandatory parameters", 500,
                        "Policy object doesn't contain mandatory parameters."),
        SUBSCRIPTION_POLICY_UPDATE_FAILED(902911, "Error while updating Subscription level policy : '%s'", 500,
                        "Server Error Occurred"),
        ...
        BLOCK_CONDITION_ALREADY_EXISTS(902916, "Block condition already exists", 409,
                        "A block condition with type: %s, value: %s already exists"),
        ALREADY_ASSIGNED_APP_POLICY_DELETE_ERROR(902912, "Cannot delete the application throttling policy", 409,
                        "Policy %s is already attached to an Application"),
        ALREADY_ASSIGNED_SUB_POLICY_DELETE_ERROR(902913, "Cannot delete the subscription throttling policy", 409,
                        "Policy %s already has subscriptions"),
        ...
        TOKEN_VALIDATION_FAILED(900989, "Validation failed for the given token", 500,
                        "Validation failed for the given token"),
        ...
        TEMPLATE_LOAD_EXCEPTION(900992, "Error in loading the template file by client as an InputStream", 500,
                        " Error " +
        ...
        DEDICATED_CONTAINER_GATEWAY_CREATION_FAILED(900995, "Error while creating dedicated container based gateway",
                        500,
                        "Error while creating dedicated container based gateway"),
        ERROR_WHILE_UPDATING_DEDICATED_CONTAINER_BASED_GATEWAY(900996,
                        "Error while updating dedicated container based" +
                                        " gateway",
                        500, "Error while updating dedicated container based gateway"),
        ...
        DEDICATED_GATEWAY_DETAILS_NOT_FOUND(900999, "Dedicated gateway details not found for the API", 404,
                        "Dedicated " +
        ...
        COMMENT_NO_PERMISSION(901104, "Insufficient permission", 403,
                        "User '%s' doesn't have permission to access the comment with id '%s'"),
        ...
        ERROR_GETTING_CUSTOM_URLS(901302, "Failed to get custom url info", 500,
                        "Error while retrieving custom url info for tenant : '%s'"),
        // Key Manager Related
        ...
        REQUIRED_KEY_MANAGER_CONFIGURATION_MISSING(901401, "Required Key Manager configuration missing", 400,
                        "Missing " +
        ...
        KEY_MANAGER_NOT_SUPPORT_OAUTH_APP_CREATION(901405, "Key Manager doesn't support generating OAuth applications",
                        400,
        ...
        KEY_MANAGER_MISSING_REQUIRED_PROPERTIES_IN_APPLICATION(901407, "Required application properties are missing",
                        400,
        ...
        DECRYPT_CONFIG_ERROR(901412, "Error while decrypting key manager configuration", 500,
        ...
        ALREADY_SUBSCRIBED_FOR_BOT_ALERTS(901604, "Subscription already exists", 409,
                        "Email: '%s' has already been subscribed for bot detection alerts"),
        BOT_DETECTION_SUBSCRIPTION_NOT_FOUND(901605, "Subscription does not exist", 404,
                        "Bot detection alert subscription with uuid: '%s' uuid does not exist"),
        ...
        TENANT_THEME_NOT_FOUND(901703, "Tenant theme does not exist", 404,
                        "Tenant theme for tenant: '%s' does not exist"),
        ...
        INVALID_CONFIGURATION_ID(900853, "The configuration id validation failed. Should be " +
                        "{apiName}#{apiVersion}#{tenantDomain}", 400,
                        "The configuration id validation failed. Should be " +
        ...
        SERVICE_IMPORT_FAILED_WITHOUT_OVERWRITE(900910, "Service import is failed", 412,
                        "Cannot update existing services " +
        ...
        API_CREATION_NOT_SUPPORTED_FOR_ASYNC_TYPE_APIS(900915,
                        "API Creation is supported only for WebSocket, WebSub and SSE APIs", 400,
        ...
        LOGGING_API_INCORRECT_LOG_LEVEL(901401, "Bad Request", 400,
                        "Log level should be either OFF, BASIC, STANDARD or FULL"),
        LOGGING_API_MISSING_DATA(901402, "Missing data", 400, "API context or log level is missing"),

        // Service Catalog related error codes
        SERVICE_VERSION_NOT_FOUND(901900, "Cannot find the service version", 404,
                        "Cannot find a service that matches the given version"),
        ...
        INVALID_TENANT_CONFIG(902001, "Invalid tenant-config found", 400, "Invalid tenant-config found with error %s",
                        false),
        ...
        INVALID_OPERATION_POLICY_PARAMETERS(902007, "Missing required parameters for operation policy specification",
                        400,
                        "Required parameter(s) %s for operation policy specification %s are either missing or empty"),
        OPERATION_POLICY_NOT_ALLOWED_IN_THE_APPLIED_FLOW(902008, "Operation policy is not allowed in the applied flow",
                        400,
        ...
        OPERATION_POLICY_SPEC_MISMATCH(902011, "Applied policy does not match specification", 400,
                        "Applied policy for URI template does not match specification"),

        OPERATION_POLICY_ALREADY_EXISTS(903001, "The Operation Policy already exists.", 409,
                        "An Operation Policy with name '%s' and version '%s' already exists"),

        OPERATION_POLICY_NOT_FOUND_WITH_NAME_AND_VERSION(903004,
                        "Operation Policy Not Found with given name and version", 404,
        ...
        OPERATION_POLICY_USAGE_EXISTS(903009, "Operation policy usages exist", 500,
                        "Policy usages exist for policy ID '%s'"),

        SUBSCRIPTION_TIER_NOT_ALLOWED(902002, "Subscription Tier is not allowed for user", 403,
                        "Subscription Tier %s is" +
                                        " not allowed for user %s ",
                        false),
        INVALID_KEY_MANAGER_REQUEST(902003, "Invalid Request sent to Key Manager.", 400,
                        "Invalid Request sent to Key Manager.Error from Backend : %s", false),
        INTERNAL_SERVER_ERROR_FROM_KEY_MANAGER(902004, "Internal Server Error from Key Manager", 500,
                        "Internal Server Error from Key Manager.Error from Backend : %s", true),
        REVISION_ALREADY_DEPLOYED(902005, "Revision deployment state conflicted", 409,
                        "Revision deployment request conflicted with the current deployment state of the revision %s. Please try again later",
                        false),
        ...
        MONETIZATION_STATE_CHANGE_FAILED(904301, "Could not change the monetization state", 500,
                        "Monetization state change to '%s' failed"),

        MONETIZATION_IMPLEMENTATION_LOADING_FAILED(904302, "Could not load the monetization implementation", 500,
                        "Failed to load the monetization implementation"),
        ...
        CERT_NOT_FOUND(904001, "Could not find the certificate", 404,
                        "'Cannot find the certificate with alias '%s' in the truststore'"),
        ...
        DELETE_CERT(904005, "Could not delete the certificate", 500,
                        "Error while deleting the certificate for alias '%s'"),
        ...
        CERT_ALREADY_EXIST(904011, "Certificate alias already exists", 409,
                        "The alias '%s' already exists in the truststore"),
        ...
        ERROR_DELETING_APPLICATION_REGISTRATION(904014, "Can not delete application registration", 400,
                        "Failed to delete Application registration of : '%s'"),

        ERROR_DELETING_APPLICATION_KEY_MAPPING(904015, "Can not delete application key mapping", 500,
                        "Failed to delete Application key mapping  of : '%s'"),

        ERROR_RETRIEVE_APPLICATION_KEYS(904016, "Failed to retrieve application keys", 500,
                        "Failed to retrieve application keys for '%s' ");
         ...
         * @param msg              The error message that you need to pass along with
         *                         the error code.
         * @param httpErrorCode    This HTTP status code which should return from REST
         *                         API layer. If you don't want to pass
        ...
        /**
         * Create an ErrorHandler instance with the provided ExceptionCode filled with
         * some dynamic input values
         *
         * @param errorHandler ErrorHandler or ExceptionCode object
         * @param params       dynamic values to be filled
         * @return ErrorHandler instance with the provided ExceptionCode filled with
         *         some dynamic input values
         */
        public static ErrorHandler from(ErrorHandler errorHandler, String... params) {
                ...
                return new ErrorItem(message, description, errorHandler.getErrorCode(),
                                errorHandler.getHttpStatusCode(),
