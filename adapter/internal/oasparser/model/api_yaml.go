...
func NewAPIYaml(fileContent []byte) (apiYaml APIYaml, err error) {
	apiJsn, err := utils.ToJSON(fileContent)
	if err != nil {
		loggers.LoggerOasparser.Errorf("Error occurred converting api file to json: %v", err.Error())
		return apiYaml, err
	}

	err = json.Unmarshal(apiJsn, &apiYaml)
	if err != nil {
		loggers.LoggerOasparser.Errorf("Error occurred while parsing api.yaml or api.json %v", err.Error())
		return apiYaml, err
	}
	...
	if err != nil {
		loggers.LoggerOasparser.Errorf("%v", err)
		return apiYaml, err
	}
	...
	if apiYaml.Data.EndpointImplementationType == constants.InlineEndpointType {
		...
		loggers.LoggerOasparser.Warnf(errmsg)
...
func (apiYaml *APIYaml) PopulateEndpointsInfo() {
	...
	if rawProdEndpoints != nil {
		if val, ok := rawProdEndpoints.(map[string]interface{}); ok {
			...
			loggers.LoggerOasparser.Warn("No production endpoints provided")
		}
	}
	...
	if rawSandEndpoints != nil {
		if val, ok := rawSandEndpoints.(map[string]interface{}); ok {
			...
			loggers.LoggerOasparser.Warn("No sandbox endpoints provided")
		}
	}
}
