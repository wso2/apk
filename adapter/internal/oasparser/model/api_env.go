...
func retrieveEndpointsFromEnv(apiHashValue string) ([]Endpoint, []Endpoint) {
	...
	for {
		...
		if err != nil {
			loggers.LoggerOasparser.Errorf("error while reading production endpoint : %v in env variables, %v", productionEndpointURL, err.Error())
	...
	for {
		...
		if err != nil {
			loggers.LoggerOasparser.Errorf("error while reading sandbox endpoint : %v in env variables, %v", sandboxEndpointURL, err.Error())
...
func resolveEnvValueForEndpointConfig(envKey string, defaultVal string) string {
	...
	if exists {
		loggers.LoggerOasparser.Debugf("resolve env value %v", envValue)
		envUnquoted, err := strconv.Unquote(envValue)
		if err != nil {
			loggers.LoggerOasparser.Debugf("Unquoting string %v in env variables has failed. %v", envValue, err.Error())
