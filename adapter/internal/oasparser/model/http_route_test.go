...
func TestConcatAPIPolicies(t *testing.T) {
	...
	dataItems := []testItem{
		...
		},
		{
			schemeUpSpec: dpv1alpha1.APIPolicySpec{
				Override: &dpv1alpha1.PolicySpec{
					RequestInterceptors: []dpv1alpha1.InterceptorConfig{
						{
				...
				Default: &dpv1alpha1.PolicySpec{
					ResponseInterceptors: []dpv1alpha1.InterceptorConfig{
						{
							BackendRef: dpv1alpha1.BackendReference{
			...
			message: `when up scheme and down scheme policies are provided, they should be combined as the result`,
		},
	}
	for _, item := range dataItems {
		schemeUp.Spec = item.schemeUpSpec
