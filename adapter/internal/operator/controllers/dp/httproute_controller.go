...
const (
	...
	conditionReasonProgrammedUnknown   gwapiv1b1.RouteConditionReason = "Unknown"
	conditionReasonConfiguredInGateway gwapiv1b1.RouteConditionReason = "ConfiguredInGateway"
	conditionReasonTranslationError    gwapiv1b1.RouteConditionReason = "TranslationError"
)
...
// NewHTTPRouteController creates a new HttpRoute controller instance. Httproute Controllers watches for gwapiv1b1.HTTPRoute.
func NewHTTPRouteController(mgr manager.Manager, operatorDataStore *synchronizer.OperatorDataStore, statusUpdater *status.UpdateHandler) error {
	...
	if err := c.Watch(source.Kind(mgr.GetCache(), &gwapiv1b1.HTTPRoute{}), &handler.EnqueueRequestForObject{},
		predicates...); err != nil {
	...
	if err := c.Watch(source.Kind(mgr.GetCache(), &gwapiv1b1.Gateway{}),
...
func (httpRouteReconciler *HTTPRouteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if supportedGatways == nil || len(supportedGatways) == 0 {
		...
		for _, gatewayWithCondition := range supportedGatways {
			gatewayParentStatus := &gwapiv1b1.RouteParentStatus{
				ParentRef: gwapiv1b1.ParentReference{
					Group:     (*gwapiv1b1.Group)(&gwapiv1b1.GroupVersion.Group),
					Kind:      common.PointerCopy(gwapiv1b1.Kind(httprouteParentKind)),
					Namespace: (*gwapiv1b1.Namespace)(&gatewayWithCondition.gateway.Namespace),
					Name:      gwapiv1b1.ObjectName(gatewayWithCondition.gateway.Name),
				},
				ControllerName: gwapiv1b1.GatewayController(GetControllerName()),
...
func getSupportedGatewaysForRoute(ctx context.Context, client k8client.Client, httpRoute gwapiv1b1.HTTPRoute) []referencedGatewaysAndCondition {
	...
	for _, parentRef := range parentRefs {
		...
		if httpRouteMatched {
			...
			if !hostnameMatched {
				...
				reason = gwapiv1b1.RouteReasonNotAllowedByListeners
			}
			var listenerName string
			if parentRef.SectionName != nil && *parentRef.SectionName != "" {
				listenerName = string(*parentRef.SectionName)
...
func checkIfRouteMatchesAllowedRoutes(
	...
	parentRefNamespace *gwapiv1b1.Namespace,
) (bool, error) {
	...
	if len(listener.AllowedRoutes.Kinds) > 0 {
		_, found := common.FindElement(listener.AllowedRoutes.Kinds, func(allowedGroupKind gwapiv1b1.RouteGroupKind) bool {
...
func checkIfMatchingReadyListenerExistsInStatus(route gwapiv1b1.HTTPRoute, listener gwapiv1b1.Listener, listenerStatuses []gwapiv1b1.ListenerStatus) error {

	// Find the relative gateway listener status for the gateway listener
	listenerStatus, listenerFound := common.FindElement(listenerStatuses, func(listenerStatus gwapiv1b1.ListenerStatus) bool {
	...
	programmedStatus, foundProgrammedStatus := common.FindElement(listenerStatus.Conditions, func(c metav1.Condition) bool {
		return c.Type == string(gwapiv1b1.ListenerConditionProgrammed)
	})
...
func isGatewayListenerHostnameMatched(listener gwapiv1b1.Listener, hostnames []gwapiv1b1.Hostname) bool {
...
func refersGateway(httpRoute gwapiv1b1.HTTPRoute, gateway gwapiv1b1.Gateway) bool {
	_, found := common.FindElement(httpRoute.Spec.ParentRefs, func(parentRef gwapiv1b1.ParentReference) bool {
		namespace := ""
		if parentRef.Namespace != nil {
			namespace = string(*parentRef.Namespace)
		}
