...
func (r *API) validateAPI() error {
	...
	if !(r.Spec.ProdHTTPRouteRef != nil && len(r.Spec.ProdHTTPRouteRef) > 0) && !(r.Spec.SandHTTPRouteRef != nil && len(r.Spec.SandHTTPRouteRef) > 0) {
	...
	if isEmptyStringsInArray(r.Spec.ProdHTTPRouteRef) {
	...
	if isEmptyStringsInArray(r.Spec.SandHTTPRouteRef) {
