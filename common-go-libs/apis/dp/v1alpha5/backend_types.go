...
type TLSConfig struct {
	...
	SecretRef *dpv1alpha2.RefConfig `json:"secretRef,omitempty"`

	// ConfigMapRef denotes the reference to the ConfigMap that contains the Certificate
	ConfigMapRef *dpv1alpha2.RefConfig `json:"configMapRef,omitempty"`
...
type SecurityConfig struct {
	...
	// AWSKey security configuration
	AWSKey *AWSKeySecurityConfig `json:"awsKey,omitempty"`
}
...
// AWSKeySecurityConfig defines AWS Key security configurations
type AWSKeySecurityConfig struct {
	// AWSService is the AWS service this security configuration is for
	Service string `json:"service,omitempty"`

    //  In is to specify how the AWS key is passed to the request
    //
    // +kubebuilder:validation:Enum=Header;Query
    // +kubebuilder:validation:MinLength=1
    In string `json:"in,omitempty"`

	// Reference for the Region of the AWS Key
    //
    // +kubebuilder:validation:MinLength=1
    RegionRef string `json:"regionRef"`

    // Reference for the Access Key of the AWSKey
    //
    // +kubebuilder:validation:MinLength=1
    AccessKeyRef string `json:"accessKeyRef"`

    // Reference for the Secret Key of the AWSKey
    // 
	// +kubebuilder:validation:MinLength=1
